export function HeartIcon(props: any) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      viewBox="0 0 24 24"
      strokeWidth={2}
      stroke="currentColor"
      {...props}
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z"
      />
    </svg>
  );
}

export function BrainIcon(props: any) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      strokeWidth="1.5"
      stroke="currentColor"
      fill="none"
      {...props}
    >
      <path stroke="none" d="M0 0h24v24H0z" fill="none" />
      <path d="M15.5 13a3.5 3.5 0 0 0 -3.5 3.5v1a3.5 3.5 0 0 0 7 0v-1.8" />
      <path d="M8.5 13a3.5 3.5 0 0 1 3.5 3.5v1a3.5 3.5 0 0 1 -7 0v-1.8" />
      <path d="M17.5 16a3.5 3.5 0 0 0 0 -7h-.5" />
      <path d="M19 9.3v-2.8a3.5 3.5 0 0 0 -7 0" />
      <path d="M6.5 16a3.5 3.5 0 0 1 0 -7h.5" />
      <path d="M5 9.3v-2.8a3.5 3.5 0 0 1 7 0v10" />
    </svg>
  );
}

export function LungsIcon(props: any) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      strokeWidth="1.5"
      stroke="currentColor"
      fill="none"
      {...props}
    >
      <path stroke="none" d="M0 0h24v24H0z" fill="none" />
      <path d="M6.081 20c1.612 0 2.919 -1.335 2.919 -2.98v-9.763c0 -.694 -.552 -1.257 -1.232 -1.257c-.205 0 -.405 .052 -.584 .15l-.13 .083c-1.46 1.059 -2.432 2.647 -3.404 5.824c-.42 1.37 -.636 2.962 -.648 4.775c-.012 1.675 1.261 3.054 2.877 3.161l.203 .007z" />
      <path d="M17.92 20c-1.613 0 -2.92 -1.335 -2.92 -2.98v-9.763c0 -.694 .552 -1.257 1.233 -1.257c.204 0 .405 .052 .584 .15l.13 .083c1.46 1.059 2.432 2.647 3.405 5.824c.42 1.37 .636 2.962 .648 4.775c.012 1.675 -1.261 3.054 -2.878 3.161l-.202 .007z" />
      <path d="M9 12a3 3 0 0 0 3 -3a3 3 0 0 0 3 3" />
      <path d="M12 4v5" />
    </svg>
  );
}

export function KidneyIcon(props: any) {
  return (
    <svg
      fill="currentColor"
      stroke="currentColor"
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512.001 512.001"
    >
      <g>
        <g>
          <g>
            <path
              d="M217.63,222.961l-27.217-23.91c25.176-17.054,38.872-44.904,38.872-73.212c0-48.759-39.669-88.429-88.429-88.429
				C63.188,37.41,0,100.599,0,178.266c0,48.759,39.669,88.429,88.429,88.429c30.231,0,58.801-15.486,75.171-41.881l29.579,25.984
				c4.165,3.658,6.553,8.935,6.553,14.479v190.789c0,10.231,8.294,18.525,18.525,18.525c10.231,0,18.525-8.294,18.525-18.525
				v-190.79C236.781,249.075,229.801,233.653,217.63,222.961z M151.813,176.065c-6.268,1.662-17.699,2.812-22.074,0.914
				c-9.385-4.076-20.296,0.228-24.371,9.615c-4.074,9.385,0.231,20.296,9.616,24.369c3.337,1.45,6.877,2.421,10.454,3.054
				c-9.405,9.749-22.576,15.627-37.01,15.627c-28.33,0-51.378-23.048-51.378-51.378c0-57.238,46.567-103.805,103.805-103.805
				c28.33,0,51.378,23.048,51.378,51.378C192.234,148.105,178.718,168.922,151.813,176.065z"
            />
            <path
              d="M371.146,37.41c-48.759,0-88.429,39.669-88.429,88.429c0,28.273,13.663,56.137,38.872,73.212l-27.217,23.91
				c-12.171,10.692-19.151,26.115-19.151,42.316v190.789c0,10.231,8.294,18.525,18.525,18.525s18.525-8.294,18.525-18.525v-190.79
				c0-5.543,2.389-10.821,6.553-14.479l29.578-25.984c16.263,26.223,44.739,41.881,75.171,41.881
				c48.759,0,88.429-39.669,88.429-88.429C512,100.599,448.813,37.41,371.146,37.41z M423.572,229.644
				c-14.435,0-27.607-5.878-37.01-15.626c3.578-0.634,7.117-1.606,10.454-3.054c9.385-4.074,13.69-14.986,9.616-24.369
				c-4.074-9.385-14.984-13.69-24.371-9.615c-4.081,1.77-14.936,0.931-22.074-0.914c-0.121-0.032-0.243-0.056-0.366-0.085
				c-0.079-0.02-0.157-0.044-0.237-0.063c-23.444-5.383-39.818-25.976-39.818-50.078c0-28.329,23.048-51.378,51.378-51.378
				c57.238,0,103.805,46.567,103.805,103.805C474.949,206.596,451.901,229.644,423.572,229.644z"
            />
          </g>
        </g>
      </g>
    </svg>
  );
}

export function AidsIcon(props: any) {
  return (
    <svg
      fill="currentColor"
      viewBox="-4 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
      <g
        id="SVGRepo_tracerCarrier"
        strokeLinecap="round"
        strokeLinejoin="round"
      ></g>
      <g id="SVGRepo_iconCarrier">
        <path d="m10.47 13.321-2.339 3.774 4.281 6.904 3.37-2.086z"></path>
        <path d="m13.555 0h-11.33l-2.226 3.97 5.313 8.58 2.339-3.781-2.972-4.8h6.418l-11.098 17.944 3.376 2.086 12.405-20.03z"></path>
      </g>
    </svg>
  );
}
